...
| 名称 | 用途 | 范围 | 来源与更新 | 风险 |
| --- | --- | --- | --- | --- |
| `VITE_SUPABASE_URL` | Supabase 项目地址 | 浏览器、构建期 | 本地 `.env.local` / Vercel / GitHub Actions Secret | 不是秘密；切换项目后需重新构建 |
| `VITE_SUPABASE_ANON_KEY` | 浏览器访问 Supabase | 浏览器、构建期 | 本地 `.env.local` / Vercel / GitHub Actions Secret | 公开客户端 key；安全完全依赖 RLS |
...
- 更换 key 或项目 URL 后，需要同步更新本地、Vercel 与 GitHub Actions Secrets，并重新部署。

## 上线前检查

- [ ] Vercel Production、Preview 与 GitHub Actions 均配置两项必需变量；
